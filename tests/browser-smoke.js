...
Exceptions: ${exceptions.join(" | ")}`);
    ...
    assert.equal(await evaluate('document.querySelector(".campaign-mission-score-grid article:last-child strong").textContent'), "1/1");
    ...
    assert.deepEqual(await evaluate('JSON.parse(localStorage.getItem("quizmon.beta1")).campaign.missionResults["pallet-town"]'), { lastFirstRunCorrect:9, bestFirstRunCorrect:9, total:10, requiredCorrect:8, lastDirectGoalMet:true, masteredMistakes:1, attempts:1 }, "The first-run score must remain available for later stars and rewards");
