(function (root, factory) {
  ...
  function dedupeFirstBy(items, keyFor) {
    const seen = new Set();
  ...
  function dedupeLatestBy(items, keyFor) {
    return dedupeFirstBy([...(Array.isArray(items) ? items : [])].reverse(), keyFor).reverse();
  }
  function byteLength(value) {
    const text = typeof value === "string" ? value : JSON.stringify(value);
    return typeof TextEncoder === "function" ? new TextEncoder().encode(text).length : text.length * 2;
  }
  ...
  return Object.freeze({
    clone, unique, finiteNonNegative, clampScore, dedupeFirstBy, dedupeLatestBy,
    byteLength, shuffle, randomItem, effectiveness, percent, formatMultiplier,
    sanitizePokemonCache, escapeHtml
  });
});
